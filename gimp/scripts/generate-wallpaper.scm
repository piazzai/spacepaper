(define (generate-wallpaper name width height filename)
    (let* ((wallpaper (car (gimp-image-new width height RGB)))
        (object (car (gimp-file-load-layer RUN-NONINTERACTIVE wallpaper (string-append (string-append "gimp/" name) ".xcf"))))
    ...
    (if (equal? name "saturn")
        (let* ((rings (car (gimp-file-load-layer RUN-NONINTERACTIVE wallpaper "gimp/rings.xcf")))
