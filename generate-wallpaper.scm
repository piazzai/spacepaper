(define (generate-wallpaper xcf width height filename)
    (let* (
        (the-image (car (gimp-image-new width height RGB)))
        (the-background (car (gimp-layer-new the-image width height RGBA-IMAGE "moon-bg" 100 LAYER-MODE-NORMAL)))
        (the-object (car (gimp-file-load-layer RUN-NONINTERACTIVE the-image (string-append (string-append "input/" xcf) ".xcf"))))
        (the-grain (car (gimp-layer-new the-image width height RGBA-IMAGE "moon-grain" 25 LAYER-MODE-GRAIN-MERGE)))
        (the-gradient (car (gimp-layer-new the-image width height RGBA-IMAGE "moon-gradient" 100 LAYER-MODE-SOFTLIGHT)))
