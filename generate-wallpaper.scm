(define (generate-wallpaper name width height)
    (let* (
        (wallpaper (car (gimp-image-new width height RGB)))
        (object (car (gimp-file-load-layer RUN-NONINTERACTIVE wallpaper (string-append (string-append "GitHub/spacepaper/objects/" name) ".xcf"))))
    ...
    (if (< width height)
        (gimp-image-raise-item-to-top wallpaper gradient-y))
...
;        ; all objects are 1700x1700 with a center on (850,850), except saturn, which is is 3304x2268 with a center on (1555,1039)
        (let* (
            (rings (car (gimp-file-load-layer RUN-NONINTERACTIVE wallpaper (string-append (string-append "GitHub/spacepaper/objects/" name) "-rings.xcf"))))
...
;    (gimp-image-delete the-image)))
    (gimp-display-new wallpaper)))
(generate-wallpaper "saturn" 800 600)
